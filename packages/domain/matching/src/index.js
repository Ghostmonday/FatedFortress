 ...
 */
export function calculateMemberPower(member) {
 ...
 */
export function calculateTeamPower(members) {
    return members.reduce((sum, m) => sum + calculateMemberPower(m), 0);
}
/**
 * Calculate specialty coverage score
 */
export function calculateSpecialtyCoverage(members) {
    const coverage = {
        BACKEND: 0,
        FRONTEND: 0,
        DEVOPS: 0,
 ...
 */
export function isTeamBalanced(members, requiredSpecialties = []) {
    ...
    for (const specialty of requiredSpecialties) {
        if (coverage[specialty] < 1)
            return false;
    }
    // Team size should be appropriate
    if (members.length < MATCHING_CONFIG.MIN_TEAM_SIZE)
        return false;
    if (members.length > MATCHING_CONFIG.MAX_TEAM_SIZE)
        return false;
    return true;
}
/**
 ...
 */
export function formTeams(members, targetTeamSize = MATCHING_CONFIG.IDEAL_TEAM_SIZE) {
    if (members.length === 0)
        return [];
 ...
 */
export function assignRoles(members) {
    if (members.length === 0)
        return [];
 ...
 */
export function formParty(members) {
