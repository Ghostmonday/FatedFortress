...
import { stakeRep, releaseStake, createTicket, claimTicket, completeTicket, processForfeitures, getStakeSummary, getTicket, listOpenTickets } from './src/index';
...
describe('Bonding Curve - Integration Tests', () => {
    ...
    describe('Full REP Flow', () => {
        ...
        it('should fail stake with insufficient REP', async () => {
            ...
            await expect(stakeRep({ actorId: A, amount: 50 })).rejects.toThrow('Insufficient REP');
        });
        it('should fail claim with insufficient staked REP', async () => {
            ...
            // Claim should fail
            await expect(claimTicket({ actorId: A, ticketId: ticket.id })).rejects.toThrow('Insufficient staked REP');
        });
        it('should fail double completion', async () => {
            ...
            await expect(completeTicket({ ticketId: ticket.id, verifierId: B })).rejects.toThrow();
        });
    });
